...
export async function shareToGroup(
  ...
  sourceUrl: string,
  groupName: string,
  postMessage: string, // kept for completeness; currently not used (original code had it commented out)
): Promise<boolean> {
  console.info(`\n📢 Sharing post to group: ${groupName}`);
  await page.waitForTimeout(2_000);

  try {
    // Navigate to the source post
    ...
    // The original Python code had the message filling commented out.
    ...
    // Wait for the post dialog to disappear
